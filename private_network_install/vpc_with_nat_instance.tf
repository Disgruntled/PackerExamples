...
resource "aws_iam_policy" "IAMPassRole" {
  name        = "IAMPassRole"
  path        = "/"
  description = "Allows The holder the ability to pass any role to any service. Not for production use!"
...
resource "aws_iam_policy_attachment" "IAM_read_only" {
  ...
  ####Actually Harmless for a change
...
resource "aws_iam_policy_attachment" "Roles_for_IAMPassRoling" {
  name  = "RolesForPassRolling"
  roles = [aws_iam_role.default_ec2_instance_policy.name]
##This is also definitely too wide to consider using outside of a short lived dev environment.
  policy_arn = aws_iam_policy.IAMPassRole.arn

}

resource "aws_iam_policy_attachment" "EC2_Access_for_builder_instance" {
  name  = "EC2_Access_for_builder_instance"
  roles = [aws_iam_role.default_ec2_instance_policy.name]
  ####Probably don't use this in prod, has way too much access
